...
parser.add_argument('--coord_normalization', type=bool, default=False, help='Coordinate normalization')
...
# Forwards automatic differentiation for SPINN
dde.config.set_default_autodiff("forward")

# =============================================================================
# 2. Utility Function: Coordinate Transformation for SPINN
# =============================================================================
...
coord_scale = [L_max, L_max] if args.coord_normalization else [1.0, 1.0]
...
def strain_from_output(x, f):
    ...
    """
    x = [x[i]*coord_scale[i] for i in range(2)] #
...
if args.measurments_type == "displacement":
    if args.DIC_dataset_path != "no_dataset":
        ...
        X_DIC_input = [np.linspace(0, x_max, args.num_measurments).reshape(-1, 1)] * 2
...
def HardBC(x, f, L_max=L_max):
    ...
    if isinstance(x, list):
        ...
        x = dde.backend.stack([x[:, 0] * coord_scale[0], x[:, 1] * coord_scale[1]], axis=1)
    Ux  = f[:, 0] * x[:, 0]/L_max * args.u_0[0] 
    Uy  = f[:, 1] * x[:, 1]/L_max * args.u_0[1]
    Sxx = f[:, 2] * (L_max - x[:, 0])/L_max + side_load(x[:, 1])
    Syy = f[:, 3] * (L_max - x[:, 1])/L_max
    Sxy = f[:, 4] * x[:, 0]/L_max * (L_max - x[:, 0])/L_max * x[:, 1]/L_max * (L_max - x[:, 1])/L_max
...
X_plot = [np.linspace(0, x_max, 100).reshape(-1, 1)] * 2
